import sys
import random
from extract_words import NMS_FILE, NFS_FILE, NMP_FILE, NFP_FILE, AMS_FILE, AFS_FILE, AMP_FILE, AFP_FILE, IS_FILE, IP_FILE
import os
...
# afp_file.close()
is_file = open(IS_FILE, 'r')
is_lines = is_file.readlines()
is_file.close()
...
for i in range(100):
    ...
    if random.choice([True, False]):
        nombre = f"{random.choice(is_lines).strip()}{random.choice(nms_lines).strip()}"
    elif random.choice([True, False]):
        nombre = f"{random.choice(is_lines).strip()}{random.choice(nfs_lines).strip()}"
    elif random.choice([True, False]):
        nombre = f"{random.choice(ip_lines).strip()}{random.choice(nmp_lines).strip()}"
    else:
        nombre = f"{random.choice(ip_lines).strip()}{random.choice(nfp_lines).strip()}"
